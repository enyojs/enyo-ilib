enyo.kind({
        ...
        {name: "panels", kind: "moon.Panels", pattern: "activity", fit: true, classes: "enyo-fill", components: [
            ...
            {title: rb.getString("First Panel"), classes: "moon-9h", titleBelow: rb.getString("First Sub-title"), subTitleBelow: rb.getString("First Sub-sub title"), components: [
                {kind: "FittableRows", components:[
                    {fit: true, components:[
                        {content: rb.getString("That is, if the translation for a string does not exist in the current locale, the more-generic parent locale is searched for the string. In the worst case scenario, the string is not found in the base locale's strings. In this case, the original source is returned as the translation. This allows developers to create code with new or changed strings in it and check in that code without waiting for the translations to be done first.")}
                    ]},
                    {components: [
            ...
            {title: rb.getString("Second Panel"), classes: "moon-9h", titleBelow: rb.getString("Second Sub-title"), subTitleBelow: rb.getString("Second Sub-sub title"), joinToPrev: true, components: [
                {kind: "FittableColumns", components: [
                    {style: "width: 33%", components: [
                        {kind: "moon.Divider", content: rb.getString("Column {num}").format({num: 1})},
                        
                        {kind: "moon.Divider", content: rb.getString("FormCheckbox Items")},
                        {kind: "moon.FormCheckbox", content: rb.getString("Option 1"), checked: true},
                        {kind: "moon.FormCheckbox", content: rb.getString("Option 2")},
                        {tag: "br"},
                        
                        {kind: "moon.Divider", content: rb.getString("Divider") + 1},
                        {kind: "moon.Item", content: rb.getString("Item 1")},
                        {kind: "moon.Item", content: rb.getString("Item 2"), selected: true},
                        {tag: "br"},
                    ...
                    {style: "width: 33%", components: [
                        ...
                        {kind: "moon.RadioItemGroup", components: [
                            ...
                            {content: rb.getString("Monte Verde Golden Toad")}
                        ]},
                        {tag: "br"},
                    ...
                    {fit: true, components: [
                        {kind: "moon.Divider", content: rb.getString("Column {num}").format({num: 3})},
                        ...
                        {kind: "moon.Accordion", content: rb.getString("This is an accordion"), components: [
                            {content: rb.getString("Item One")},
                            {content: rb.getString("Item Two")}
                        ]},
                        {tag: "br"},
    ...
    setupItem: function(inSender, inEvent) {
        var i = inEvent.index;
        this.$.sampleMoonItem.setContent(rb.getString("Item {num}").format({num: i+1}));
    }
});
