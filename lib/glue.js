...
i18n.$L.extend(function (sup) {
	return function (string) {
		var str;
		if (!i18n.$L.rb) setLocale();
		if (typeof(string) === 'string') {
			str = i18n.$L.rb ? i18n.$L.rb.getString(string) : string;
		} else if (typeof(string) === 'object') {
			if (typeof(string.key) !== 'undefined' && typeof(string.value) !== 'undefined') {
				str = i18n.$L.rb ? i18n.$L.rb.getString(string.value, string.key) : string.value;
			} else {
				str = '';
			}
		} else {
...
function setLocale (spec) {
	...
	if (!i18n.$L.rb || spec !== i18n.$L.rb.getLocale().getSpec()) {
		// Set the ilib.ResBundle for the active locale used by $L
		i18n.$L.rb = new ResBundle({
			locale: locale,
			type: 'html',
			name: 'strings',
			sync: true,
